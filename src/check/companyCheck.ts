...
export async function companyCheck(
  ...
  const some = (
    await axios.get(
      `http://111.229.188.40:3000/api/companies/name/search/${py}`
    )
  ...
  if (checkResult === false) {
    ...
    if (id === null) {
      ...
      console.log(`模型认为不存在匹配的 Company`);
    } else {
      // 存在匹配的公司名称
      ...
      console.log(`存在匹配的 Company: ${ret.value}`);
    }
  }
